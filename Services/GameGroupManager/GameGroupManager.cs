...
internal class GameGroupManager : IGameGroupManager
{
    ...
    public GameGroupManager(string extensionsDataPath)
    {
        ...
        IList<GameGroup>? gameGroups = null;
        if (File.Exists(_dataPath))
        {
            gameGroups = JsonSerializer.Deserialize<IList<GameGroup>>(File.ReadAllText(_dataPath));
        }
        _gameGroups = gameGroups ?? [];

        GameGroups = new ReadOnlyCollection<GameGroup>(_gameGroups);
    }

    #endregion

    #region Interface
    public IReadOnlyCollection<GameGroup> GameGroups { get; private init; }
    public void CreateGameGroup(GameGroup gameGroup) => CreateGroup(gameGroup);
    public void AddGamesToGroup(GameGroup gameGroup, IEnumerable<string> gameGuids) => AddGames(gameGroup, gameGuids);
    ...
    public void RemoveGamesFromGroups(IEnumerable<string> gameGuids) => RemoveFromGroups(gameGuids);
    public void      RemoveGamesFromGroup        (GameGroup         gameGroup, IEnumerable<string>    gameGuids) => RemoveFromGroup         (gameGroup,    gameGuids);
    ...
    private void AddGames(GameGroup gameGroup, IEnumerable<string> gameGuids)
    {
    ...
    private void RemoveFromGroups(IEnumerable<string> gameGuids)
    {
    ...
    private void RemoveFromGroup(GameGroup gameGroup, IEnumerable<string> gameGuids)
    {
    ...
    private static bool RemoveGuidsFromGroup(GameGroup group, IEnumerable<string> guids)
    {
